    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input:focus, select:focus {
            ...
            box-shadow: 0 0 0 3px var(--main-glow);
        }
        ...
        .btn-primary {
            ...
            color: white;
            box-shadow: 0 4px 14px var(--main-glow);
        }
        ...
        .stat-box:nth-child(4) { animation-delay: 0.2s; border: 2px solid var(--warning); }
        ...
        /* TABLE */
        .table-wrap { max-height: 250px; overflow-y: auto; border-radius: 10px; border: 1px solid var(--bg2); }
        ...
        /* MINI GAUGE in bilan */
        .gauge-bars { margin-top: 18px; }
        .gauge-row { display: grid; grid-template-columns: 120px 1fr 60px; align-items: center; gap: 10px; margin-bottom: 10px; }
        .gauge-row .gauge-label { font-size: 0.82rem; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .gauge-row .gauge-bar-bg { background: var(--bg2); height: 10px; border-radius: 5px; overflow: hidden; }
        .gauge-row .gauge-bar-fill { height: 100%; border-radius: 5px; transition: width 1s cubic-bezier(.4,0,.2,1); }
        .gauge-row .gauge-info { font-size: 0.8rem; color: var(--text-muted); text-align: right; }
...
<div class="container">
    <header>
        ...
        <div class="header-actions">
            <button class="theme-toggle" id="themeToggle" onclick="toggleTheme()" title="Changer de thème"></button>
            <button class="btn-secondary" onclick="resetMois()" style="width:auto; padding: 10px 18px;">📁 Clôturer le mois</button>
        </div>
    </header>
    ...
    <div class="stat-grid">
        ...
        <div class="stat-box" id="view_solde_box">
...
<script>
    ...
    function ajouterDepense() {
        ...
        if(!desc || isNaN(mt) || mt <= 0) {
            showToast('Remplis tous les champs correctement', 'warning');
            return;
        }
    ...
    function majAffichage() {
        ...
        // Animate stats
        const soldeEl = document.getElementById('view_solde');
        ...
        animateValue(soldeEl, prevStats.solde, solde, 600);
        ...
        db.categories.forEach((cat, i) => {
            ...
            const status = !over 
                ? '<span class="status-badge bg-success">✓ OK</span>' 
                : '<span class="status-badge bg-danger">⚠ DÉPASSÉ</span>';
    ...
    function resetMois() {
        if(confirm("Clôturer le mois ? L'épargne du mois sera ajoutée au coffre-fort. Les dépenses habituelles seront vidées, mais les dépenses récurrentes seront conservées.")) {
            let epargneMois = 0;
            ...
            sauvegarder();
            showToast(`Mois clôturé ! +${epargneMois}€ ajoutés au coffre-fort`, 'success', 4000);
        }
    }
